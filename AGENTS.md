...
- INV-6 수정주가를 쓴다
- INV-7 **시계열 구조** — ① 상승 시작 저점 → ② 20일 고점 → ③ 눌림 바닥 → ④ 오늘(반등)
  순서를 만족해야 한다. 되돌림비율 분모는 ①→② 구간(`H − L_leg`)이고,
  오늘이 눌림 바닥이면(`days_since_pullback_low = 0`) 반등 미확인이므로 탈락

### 되돌림은 "값"이 아니라 "순서"다

지표가 밴드 안에 있는지만 보면 안 된다. 가격이 ①②③④를 그 순서대로 지나왔는지
확인해야 한다. 값 조건만 검사하다 R-1(고점 다음날)·R-2(오늘이 눌림 바닥)가 통과했다.
새 조건을 넣을 때도 "이게 경로의 어느 단계를 검증하는가"를 먼저 답하라.
